...
import { currency, PV, PMT } from "@/lib/utils";
...
export default function Save() {
  ...
  const pv = PV(values.rate / 100 / values.compounding, values.nper * values.compounding, 0, values.fv) ?? 0 * -1;
  const pmt =
    -PMT(values.rate / 100 / values.compoundingAdd, values.nperAdd * values.compoundingAdd, -values.pvAdd, values.fvAdd) *
  ...
  return (
    <Container>
      ...
      <b>
        How much do I need to save/invest now to make a <u>certain goal</u> in the future assuming a <u>certain rate of return?</u>
      </b>
      <Form>
      ...
      <Row>
        <Col sm={8}>How much do I need to invest now?</Col>
        <Col sm={4}>{currency(pv)}</Col>
      </Row>
      <hr />
      ...
      <Row>
        <Col sm={8}>Additional amount needed to be saved/invested annually</Col>
